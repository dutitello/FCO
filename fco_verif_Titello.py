...
#
# Criando um novo dataframe do pandas
#
...
with pd.ExcelWriter('dados_novaaba.xlsx', engine='openpyxl') as writer:
    sheet.to_excel(writer, sheet_name='Planilha1', index=False)
    dfres.to_excel(writer, sheet_name='Resultados', index=False)


#-----------------------------------------------------------------------------
# Se quiser juntar após a última coluna precisa concatenar os dataframes e gravar direto
#       Aqui grava direto pois é apenas um dataframe
dfjunto = pd.concat([sheet, dfres], 1)
dfjunto.to_excel('dados_aofim.xlsx', engine='openpyxl', index=False)
#-----------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------
#
# Insere figura do Python no excel - No arquivo com a nova aba para resultados
#
# Pra isso precisa usar a biblioteca openpyxl pois o pandas é apenas para dados
import openpyxl
# Abre o arquivo e a aba de trabalho
wb = openpyxl.load_workbook('dados_novaaba.xlsx')
ws = wb['Resultados']

# Criar objeto de figura no openpyxl com a figura salva pelo figure antes (linha 460)
img = openpyxl.drawing.image.Image('diagrama.png')

# Insere a figura na celula F2 (pra dar uma folga)
ws.add_image(img, 'F2')

# Grava arquivo
wb.save('dados_novaaba.xlsx')
...
#-----------------------------------------------------------------------------
#
# Cria grafico do excel com pontos inseridos - No arquivo onde todos ficam juntos
#
# De novo usando a openpyxl
import openpyxl
import openpyxl.chart 

# Abre o arquivo e a aba de trabalho atual (só tem 1 nesse arquivo)
wb = openpyxl.load_workbook('dados_aofim.xlsx')
ws = wb.active

# Grafico tipo Scatter
graf = openpyxl.chart.ScatterChart()

# Não sei qual definição desses estilos... 7 pq é meu aniversário :)
graf.style = 7

# Eixos e tamanho
graf.x_axis.title = 'Mxrd'
graf.y_axis.title = 'Myrd'
graf.height = 20
graf.width = 20

# Como só tem uma série desliguei a legenda. Na série tem title_from_data=True e os valores de Y são
#   pegos desde a primeira linha, onde está o nome a ser exibido na legenda caso ligado
graf.legend = None

# Valores de X da coluna 21 da linha 2 até o fim (tamanho+1)
xval = openpyxl.chart.Reference(ws, min_col=21, min_row=2, max_row=(1+np.size(res['mixd'])))

# Valores de Y na coluna 22 da linha 1 (titulo da séria) até o fim (tamanho+1)
yval = openpyxl.chart.Reference(ws, min_col=22, min_row=1, max_row=(1+np.size(res['mixd'])))

# Cria série com xval e yval e anexa ao grafico
serie = openpyxl.chart.Series(yval, xval, title_from_data=True)
graf.series.append(serie)

# Insere grafico no excel celular B11
ws.add_chart(graf, "B11")

# Grava arquivo
wb.save('dados_aofim.xlsx')
